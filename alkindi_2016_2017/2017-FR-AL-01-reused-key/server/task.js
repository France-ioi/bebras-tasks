...
module.exports = {
        ...
        taskData: (args, callback) => {
            ...
            callback(null, full_task);
        },

        
        taskHintData: (args, callback) => {
            const {keyIndex} = args.task.hint_requested;
            const {secretKey} = args.task; //?????

            const hints = {}
            args.task.hint_requested.forEach(function(v,idx) {
                if(typeof idx !== 'number' || idx < 0 || idx >= secretKey.length) {
                    return callback(new Error('Wrong key index'));
                }
                hints[idx] = secretKey[idx]
            })
            callback(null, hints);
        ...
        gradeAnswer: (args, callback) => {
            ...
            callback(null, {
                score: nCorrect == secretKey.length ? args.max_score : args.min_score,
                message: ''
            })
        }
    }