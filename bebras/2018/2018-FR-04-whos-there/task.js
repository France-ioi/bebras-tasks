function initTask(subTask) {
   ...
   function initState() {
      ...
      if (level != "medium") {
        ...
        for (var i = 0; i < length; i++) {
          // one chance in two to leave the house, up to the quota
   ...
   var start = function(event) {
      subTask.reloadAnswerObject(subTask.getDefaultAnswerObject());
   	errorState = false;
   ...
   function saveAnswer() {
      canvas = JSON.parse(sketchpad.json());
   };
